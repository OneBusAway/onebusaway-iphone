...
const CGFloat ISHHoverBarDefaultItemDimension = 40.0;
...
- (void)updateShadowPathAndMask {
    ...
    CGFloat outsetForShadow = -1.f * (ABS(self.shadowOffset.height) + ABS(self.shadowOffset.width) + self.shadowRadius * 2.f);
    ...
    [maskShapeLayer setFrame:self.bounds];
    [maskShapeLayer setPath:[maskPath CGPath]];
}
...
- (void)drawRect:(CGRect)rect {
    if (self.separatorWidth == 0 || !self.separatorColor) {
        return;
    }
    ...
    CGFloat separatorOffset = self.separatorWidth < 1.f ? self.separatorWidth / 2.f : 0.f;
...
- (void)commonInit {
    ...
    [self setBorderWidth:1.f / [[UIScreen mainScreen] scale]];
    [self setCornerRadius:8.f];
    [self setShadowOpacity:0.25f];
    [self setShadowColor:[UIColor blackColor]];
    [self setShadowRadius:4.f];
}
